...
# Read the CSV data
data = pd.read_csv('D:\Centennial College - Subject Materials\SEM 2\COMP 247 AI\AI_Project_Final\AI_Project_Final\BackEnd\KSI.csv')
...
data.isna().sum()
data.corr()
path = r'D:\Centennial College - Subject Materials\SEM 2\COMP 247 AI\AI_Project_Final\AI_Project_Final\BackEnd\project_files'
...
x = [total_2006, total_2007, total_2008, total_2009, total_2010, total_2011, total_2012, total_2013, total_2014,
     total_2015, total_2016, total_2017, total_2018, total_2019, total_2020, total_2021, total_2022, total_2023]
...
data['INJURY'] = data['INJURY'].map(
    {'None': 0, 'Non-Fatal Injury': 0.25, 'Minor': 0.50, 'Major': 0.75, 'Fatal': 1})
data['INJURY']
...
area = pd.DataFrame()
area['Etobicoke'] = data['YEAR'][data['DISTRICT']
                                 == 'Etobicoke York'].value_counts()
area['NorthYork'] = data['YEAR'][data['DISTRICT'] == 'North York'].value_counts()
area['Scarborough'] = data['YEAR'][data['DISTRICT']
                                   == 'Scarborough'].value_counts()
area['Toronto&EastYork'] = data['YEAR'][data['DISTRICT']
                                        == 'Toronto and East York'].value_counts()
...
# check the columns with high null percentage and drop them.
for column in data.columns:
    null_percentage = (data[column].isnull().sum() / len(data)) * 100
    print("Null percentage for column {} - {:.2f}%".format(column, null_percentage))
...
def invtype_replace(data):
    driver_types = ['Driver', 'Truck Driver',
                    'Motorcycle Driver', 'Moped Driver']
...
data = data.drop(columns=['NEIGHBOURHOOD_158',
                 'NEIGHBOURHOOD_140', 'STREET1', 'STREET2'])
# Reset index
...
StratifiedShuffleSplit = StratifiedShuffleSplit(
    n_splits=1, test_size=0.2, random_state=76)  # splt the data 20% test
...
'''
clf = ImbPipeline(steps=[('full_pipeline', preprocessing_pipeline(X_train)),
...
feature_names = clf.named_steps['full_pipeline'].transformers_[0][2] + \
    list(clf.named_steps['full_pipeline'].named_transformers_[
...
print("Train Accuracy:", accuracy_score(
    y_train, clf.predict(X_train)))  # training accuracy
print("Test Accuracy:", accuracy_score(
    y_test, clf.predict(X_test)))  # testing accuracy
...
for classifier_name, classifier in classifiers.items():
    ...
    pipeline = ImbPipeline(steps=[('new_pipeline', new_pre_processing_pipeline),
    ...
    print(f"Best {classifier_name} Train Accuracy:",
          accuracy_score(y_train, best_estimator.predict(X_train)))
    print(f"Best {classifier_name} Test Accuracy:",
    ...
    joblib.dump(best_estimator, path +
                f'\{classifier_name.lower().replace(" ", "_")}_best_estimator.pkl')
